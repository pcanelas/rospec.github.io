...
from src.rospec.language.ttypes import TType, OptionalType, BasicType
...
class ArrayAccess(Expression):
    ...
    def __hash__(self):
        return 31 * hash("ArrayAccess") + 31 * hash(tuple(
            self.indexes)) + 31 * hash(self.target)
...
class FunctionCall(Expression):
    ...
    def __hash__(self):
        return 31 * hash("FunctionCall") + 31 * hash(
            self.operator) + 31 * hash(tuple(self.operands))

    def __str__(self):
        if len(self.operands
               ) == 1 and self.operator.name == '-':  # if it is is_unary
...
class ConfigurableInformation(Statement):
    ...
    def __str__(self):
        is_optional = "optional " if isinstance(self.ttype,
                                                OptionalType) else ""
...
class NodeType(ROSpecNode):
    ...
    configurable_information: list[ConfigurableInformation] = field(
