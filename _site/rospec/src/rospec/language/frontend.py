...
from src.rospec.language.nodes import Program, ConfigurableInformation, Remapping, Identifier, Literal, FunctionCall, \
    Expression, PolicyInstance, Message, PolicyAttached, Publisher, Subscriber, \
    ServiceActionRole, Service, TFTransform, Action, NodeType, PluginType, PluginInstance, Array, \
    NodeInstance, MessageAlias, TypeAlias, System, ConfigurationType, TransformType, Field, FieldType, ArrayAccess

from src.rospec.language.ttypes import t_bottom, TType, RefinedType, EnumType, ArrayType, BasicType, \
    OptionalType, t_bool_bool, t_comparison_type, t_bool_bool_bool, t_float_float_float, t_string, StructType
from src.rospec.language.utils import convert_literal, filter_nodes_by_type, \
    aux_node_plugin_fields_constructor, infer_qos_type, merge_and_dependencies, replace_empty_in_attributes
from src.rospec.verification.substitution import inverse_substitution_expr_expr, inverse_expr_substitution_in_type
...
class TreeToROSpec(Interpreter):
    ...
    def system(self, args):
        node_instances = filter_nodes_by_type(NodeInstance, args)
        plugin_instances = filter_nodes_by_type(PluginInstance, args)
        return System(node_instances=node_instances,
    ...
    def policy_instance(self, args):
        ...
        return PolicyInstance(instance_name=name,
    ...
    def plugin_type(self, args):
        name: Identifier = args[0]
        attributes, dependencies = aux_node_plugin_fields_constructor(
            args[1:], name.name)
        dependency: Expression = merge_and_dependencies(dependencies)
        dependency = inverse_substitution_expr_expr(old='_',
                                                    new=name,
                                                    expr=dependency)
    ...
    def message_alias(self, args):
        ...
        fields: list[ConfigurableInformation] = filter_nodes_by_type(
            Field, args[2:])
        dependencies: list[Expression] = args[2 + len(fields):]
        dependency: Expression = merge_and_dependencies(dependencies)
        return MessageAlias(new_ttype=new_ttype,
                            old_ttype=old_ttype,
                            fields=fields,
                            dependency=dependency)
    ...
    def message_type(self, args):
        ...
        ttype = StructType(
            fields={field.identifier.name: field.ttype
                    for field in fields})
        return TypeAlias(new_ttype=new_ttype,
                         old_ttype=ttype,
                         dependency=dependency)
    ...
    def configurable_type(self, args):
        is_optional: bool = isinstance(args[0],
                                       Token) and args[0].type == 'OPTIONAL'

        # This ensures that we always have the right number of arguments
        if len(args) == 4:
            if not is_optional:
                raise ValueError(
                    "Attempted to provide default value for non-optional.")
            else:
    ...
    def configurable_instance(self, args):
        configuration_type: ConfigurationType = ConfigurationType(
            args[0].value)
        identifier: Identifier = args[1]
    ...
    def connection(self, args):
        ...
        if isinstance(topic, Identifier):
            ...
            topic = FunctionCall(operator=topic.operator,
                                 operands=copy.deepcopy(topic.operands),
                                 ttype=ttype)
        else:
        ...
        return connection_constructor(node=Identifier(name='_',
                                                      ttype=t_bottom),
                                      topic=topic,
                                      policies=None)
    ...
    def service_action(self, args):
        ...
        if isinstance(expr, Identifier):
            ...
            topic = FunctionCall(operator=expr.operator,
                                 operands=copy.deepcopy(expr.operands),
                                 ttype=topic_ttype)
        else:
            raise ValueError(f"Unexpected topic type: {type(expr)}")
        return service_action_constructor(role=role,
    ...
    def frames(self, args):
        return TFTransform(transform=TransformType(args[0].value),
                           parent_frame=args[1],
                           child_frame=args[2],
    ...
    def remapping(self, args):
        return Remapping(node=Identifier(name="_", ttype=t_bottom),
                         remap_from=args[0],
                         remap_to=args[1])

    @visit_children_decor
    def ttype(self, args) -> TType:
    ...
    def refined_type(self, args):
        return RefinedType(name=Identifier(name='_', ttype=args[0]),
                           ttype=args[0],
                           refinement=args[1])
    ...
    @visit_children_decor
    def array_type(self, args) -> ArrayType:
        basic_type: BasicType = BasicType(args[0].value)
        literals: list[tuple] = [
            convert_literal(x.value, x.type) for x in args[1:]
        ]
    ...
    @visit_children_decor
    def condition(self, args) -> Expression:
        return args[0]

    @visit_children_decor
    def parens_exr(self, args) -> Expression:
        return args[0]

    @visit_children_decor
    def expr(self, args) -> Expression:
        return args[0]
    ...
    @visit_children_decor
    def not_expr(self, args) -> FunctionCall:
        operand = Identifier(name=args[0].value, ttype=t_bool_bool)
        return FunctionCall(operator=operand,
                            operands=[args[1]],
                            ttype=t_bool_bool)

    @visit_children_decor
    def minus_expr(self, args) -> Union[Literal, FunctionCall]:
        if isinstance(args[1], Literal):
            return Literal(value=-args[1].value, ttype=args[1].ttype)
        return FunctionCall(operator=args[0],
                            operands=[args[1]],
                            ttype=args[1].ttype)

    @visit_children_decor
    def arithmetic_expr(self, args) -> FunctionCall:
        return FunctionCall(operator=Identifier(name=args[1].value,
    ...
    @visit_children_decor
    def invocation_expr(self, args) -> FunctionCall:
    ...
    @visit_children_decor
    def variable_expr(self, args) -> Identifier:
        return args[0]
