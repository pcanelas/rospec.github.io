...
from src.rospec.language.nodes import ROSpecNode, Expression, Identifier, FunctionCall, \
    Publisher, Subscriber, Action, PolicyAttached, TFTransform, Service, ConfigurableInformation, Remapping
from src.rospec.language.ttypes import t_bool, t_float, t_int, t_string, t_double, TType, EnumType, \
    t_bool_bool_bool, t_bottom


def convert_literal(value: str,
                    ttype: str) -> Tuple[Union[int, float, bool, str], TType]:
...
def aux_node_plugin_fields_constructor(args, node_name):
    ...
    for arg in args:
        if isinstance(arg, PolicyAttached):
            ...
            if type(arg) in [
                    Publisher, Subscriber, Service, Action, TFTransform,
                    Remapping
            ]:
...
def infer_qos_type(identifier: Identifier) -> TType:
    ...
    dispatcher = {
        ...
        EnumType(ttypes=[
            ...
            Identifier(name='Manual', ttype=t_bottom)
        ]),
        'depth':
        t_int,
        'deadline':
...
def merge_and_dependencies(
        dependencies: list[Expression]) -> Optional[Expression]:
    ...
    return FunctionCall(
        ...
        operands=[dependencies[0],
                  merge_and_dependencies(dependencies[1:])])
