...
import src.rospec.language.nodes as rospec_nodes
...
class ArrayType(TType):
    ...
    def __eq__(self, other):
        return isinstance(
            other, ArrayType
        ) and self.ttype == other.ttype and self.max_size_refinement == other.max_size_refinement

    def __hash__(self):
        return 31 * hash(self.ttype) and 31 * hash(
            self.max_size_refinement) and 31 * self.max_size_refinement
...
class RefinedType(TType):
    ...
    def __eq__(self, other):
        return isinstance(
            other, RefinedType
        ) and self.ttype == other.ttype and self.refinement == other.refinement
...
class OptionalType(TType):
    ...
    def __eq__(self, other):
        return isinstance(
            other, OptionalType
        ) and self.ttype == other.ttype and self.default_value == other.default_value
...
class NodeT(TType):
    fields: StructType
    connections: typing.List[typing.Union[rospec_nodes.Publisher,
                                          rospec_nodes.Subscriber,
                                          rospec_nodes.Service,
                                          rospec_nodes.Action]]
    frames: typing.List[rospec_nodes.TFTransform]

    def __eq__(self, other):
        return (isinstance(other, NodeT) and self.fields == other.fields
                and self.connections == other.connections)

    def __hash__(self):
        return 31 * hash("Node") + 31 * hash(self.fields) + 31 * hash(
            self.connections)
    ...
    def __str__(self):
        fields_str = ', '.join(
            [f'{k}: {v}' for k, v in self.fields.fields.items()])
...
class PluginT(TType):
    ...
    def __eq__(self, other):
        return (isinstance(other, PluginT) and self.fields == other.fields
                and self.connections == other.connections)

    def __hash__(self):
        return 31 * hash("PluginT") + 31 * hash(self.fields) + 31 * hash(
            self.connections)
...
class AbstractionType(TType):
    ...
    def __eq__(self, other):
        return isinstance(
            other, AbstractionType
        ) and self.argument == other.argument and self.result_type == other.result_type
...
t_comparison_type = AbstractionType(t_bottom,
                                    AbstractionType(t_bottom, t_bool))
t_bool_bool_bool = AbstractionType(t_bool, AbstractionType(t_bool, t_bool))
t_bool_bool = AbstractionType(t_bool, t_bool)

t_float_float_float = AbstractionType(t_float,
                                      AbstractionType(t_float, t_float))
t_float_float_bool = AbstractionType(t_float, AbstractionType(t_float, t_bool))

default_types = [
    t_bottom, t_bool, t_string, t_plugin, t_int, t_uint8, t_int32, t_uint32,
    t_int64, t_float, t_double, t_float32, t_float64
]
