...
from src.rospec.language.nodes import Statement, ConfigurableInformation, Expression, Field, Remapping, Action, \
    Service, Subscriber, Publisher, TFTransform, Identifier
from src.rospec.language.ttypes import TType, OptionalType
from src.rospec.verification.context import Context
from src.rospec.verification.expression_formation import expr_formation
...
def spd_configuration(_: Context,
                      config: ConfigurableInformation) -> (str, TType):
    return config.identifier.name, config.ttype


def spd_opt_configuration(context: Context,
                          config: ConfigurableInformation) -> (str, TType):
...
def spi_configuration(context: Context,
                      config: ConfigurableInformation) -> (str, Expression):
