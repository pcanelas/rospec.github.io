from src.rospec.language.nodes import Literal, Expression, FunctionCall, Identifier, Message, Array, ArrayAccess
from src.rospec.language.ttypes import t_int, t_float, t_string, t_bool, StructType, ArrayType, AbstractionType, \
    BasicType, TType
...
def e_literal(context: Context, literal: Literal):
    ...
    assert is_subtype(context, u,
                      dispatcher[t]), f"Type {u} not a subtype of {t}"
...
def e_exp_bin_op(context: Context, exp_bin: FunctionCall):
    assert exp_bin.operator.name in [
        '>', '<', '>=', '<=', '==', '!='
    ], f"Operator {exp_bin.operator.name} not allowed"
    ...
    assert is_subtype(context, e_1.ttype, t_int) or is_subtype(
        context, e_1.ttype, t_float), f"Operands {e_1} not int or float"
    assert is_subtype(context, e_2.ttype, t_int) or is_subtype(
        context, e_2.ttype, t_float), f"Operands {e_2} not int or float"


def e_bool_bin_op(context: Context, bin_op: FunctionCall):
    assert bin_op.operator.name in [
        'and', 'or', '->'
    ], f"Operator {bin_op.operator.name} not allowed"
...
def e_message(context: Context, message: Message):
    ...
    assert isinstance(message.ttype,
                      BasicType), f"Type {message.ttype} not a basic type"
    ...
    assert is_subtype(
        context, StructType(instantiated_struct),
        t_struct), f"Type {t_struct} not a subtype of {instantiated_struct}"
...
def e_array_access(context: Context, array_access: ArrayAccess):
    ...
    for index in array_access.indexes:
        ...
        assert is_subtype(context, index.ttype,
                          t_int), f"Index {index} not an integer"
    ...
    for i in range(number_of_accesses):
        assert isinstance(
            target_type,
            ArrayType), f"Target type {target_type} not an array type"
        target_type = target_type.ttype

    assert is_subtype(
        context, target_type, array_access.ttype
...
def e_call(context: Context, call: FunctionCall):
    ...
    assert isinstance(
        call_ttype,
        AbstractionType), f"Type {call_ttype} not a type of a function"
    ...
    for argument in call.operands:
        ...
        assert is_subtype(
            context, argument.ttype, call_ttype.argument
    ...
    assert is_subtype(
        context, call.ttype,
        call_ttype), f"Type {call.ttype} not a subtype of {call_ttype}"
