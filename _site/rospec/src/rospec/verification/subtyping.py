...
from src.rospec.language.ttypes import TType, t_bottom, StructType, ArrayType, EnumType, AbstractionType, \
    OptionalType, RefinedType, BasicType, t_float, t_int, t_double, t_string, t_float64
from src.rospec.verification.context import Context
from src.rospec.verification.interpreter import interpret
from src.rospec.verification.substitution import inverse_substitution_expr_expr
...
def is_subtype_refined(context: Context, t: TType, u: TType) -> bool:
    if isinstance(t, RefinedType) and isinstance(u, RefinedType):
        ...
        assert isinstance(
            t.refinement, rospec_nodes.FunctionCall
        ) and t.refinement.operator.name == "==", f"{t.refinement} not =="
        value = t.refinement.operands[
            1]  # TODO: Assuming that the right hand side is always already interpreted
        new_expr = inverse_substitution_expr_expr(u.name.name, value,
                                                  u.refinement)
        is_satisfied = interpret(context, new_expr)
        assert isinstance(is_satisfied, bool)
        if not is_satisfied:
            context.add_error(
                f"Refinement {t.refinement} not satisfied in {u}")
...
def is_subtype_struct(context: Context, t: StructType, u: StructType) -> bool:
    ...
    for key in t.fields.keys():
        if key not in u.fields:
            context.add_error(
                f">> Struct {t} is not a subtype of {u}, because the field {key} is not present in {u}"
            )
            result = False
    ...
    for u_key, u_value in u.fields.items():
        if not isinstance(u_value, OptionalType) and u_key not in t.fields:
            context.add_error(
                f"Struct {t} is not a subtype of {u}, because the field {u_key} is not present in {t}"
            )
            result = False
...
def is_subtype(context: Context, t: TType, u: TType) -> bool:
    ...
    if t in [t_int, t_float, t_double, t_float64
             ] and u in [t_float, t_double, t_float64]:
    ...
    if isinstance(t, ArrayType):
        return isinstance(
            u, ArrayType
        ) and t.max_size_refinement == u.max_size_refinement and is_subtype(
            context, t.ttype, u.ttype)
    ...
    if isinstance(t, EnumType) and isinstance(u, EnumType):
        ...
        return isinstance(u, EnumType) and all(t_elem in u.ttypes
                                               for t_elem in t.ttypes)
