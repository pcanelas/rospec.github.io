...
from src.rospec.language.nodes import ROSpecNode, TypeAlias, MessageAlias, PolicyInstance, NodeType, NodeInstance, \
    PluginType, PluginInstance, System, Program, Publisher, Subscriber, Service, Action, TFTransform, \
    Connection, ServiceActionRole, TransformType, Literal
from src.rospec.language.ttypes import StructType, BasicType, TType, RefinedType, OptionalType, NodeT, t_bottom, \
    EnumType, PluginT
from src.rospec.verification.context import Context
from src.rospec.verification.interpreter import interpret, interpret_connection
from src.rospec.verification.statement_formation import st_formation
from src.rospec.verification.substitution import substitute_connection_with_assignments, \
    substitute_connection_with_remaps, selfification, inverse_expr_substitution_in_type
from src.rospec.verification.subtyping import is_subtype
from src.rospec.verification.type_formation import ty_formation
from src.rospec.verification.utils import convert_to_expression, check_policies
...
def d_message_alias(context: Context, msg_alias: MessageAlias) -> Context:
    ...
    if not set([f.identifier.name for f in msg_alias.fields]) == set(
            old_ty_struct.fields.keys()):
    ...
    new_ty_struct = StructType(
        {f.identifier.name: f.ttype
         for f in msg_alias.fields})

    return context.add_alias(msg_alias.new_ttype, new_ty_struct)


def d_policy_instance(context: Context,
                      policy_instance: PolicyInstance) -> Context:
    ...
    for parameter in policy_instance.parameters:
        if parameter.identifier.name not in policy_struct.fields:
            context = context.add_error(
                f"Field {parameter.identifier.name} not found in {policy_struct.fields}"
            )
        else:
            parameter_ttype = selfification(parameter.value,
                                            name=parameter.identifier.name)
            if not is_subtype(context, parameter_ttype,
                              policy_struct.fields[parameter.identifier.name]):
                policy_field_ttype = policy_struct.fields[
    ...
    new_ty_struct = StructType({
        f.identifier.name:
        OptionalType(policy_struct.fields[f.identifier.name], f.value)
        for f in policy_instance.parameters
    })

    return context.add_typing(policy_instance.instance_name.name,
                              new_ty_struct)
...
def d_node_type(context: Context, node_type: NodeType) -> Context:
    ...
    for config in node_type.configurable_information:
        ...
        if isinstance(config_ty, OptionalType):
            ...
            if isinstance(config_ty.default_value, Literal) and isinstance(
                    new_ttype_replaced, RefinedType):
                if not interpret(context=context,
                                 expr=new_ttype_replaced.refinement):
                    context.add_error(
                        f'Refinement {new_ttype_replaced.refinement} not satisfied in {config_ty.default_value}'
                    )

    connections: list[Union[Publisher, Subscriber, Service, Action,
                            TFTransform]] = []
    ...
    if node_type.dependency is not None:
        nodety = RefinedType(name=node_type.name,
                             ttype=nodety,
                             refinement=node_type.dependency)
...
def d_node_instance(context: Context, node_instance: NodeInstance) -> Context:
    ...
    if node_instance.name.name in context.typing:
        return context.add_error(
            f"Node {node_instance.name.name} already defined in context")
    ...
    assert isinstance(x2_node_type, NodeT) or isinstance(
        x2_node_type, RefinedType)
    ...
    for config in node_instance.configurable_information:
        ...
        internal_context = internal_context.add_value(
            st_config_name, interpret(internal_context, st_config_expr))
    ...
    node_instance_struct: StructType = StructType({
        field_name:
        selfification(
            convert_to_expression(context, val,
                                  internal_context.get_typing(field_name)),
            field_name)
        for field_name, val in internal_context.values.items()
    })

    if not is_subtype(internal_context, node_instance_struct,
                      x2_node_type.fields):
        context = context.add_error(
    ...
    for y2, t2 in x2_node_type.fields.fields.items():
        if isinstance(t2, OptionalType):
            ...
            if y2 not in internal_context.values:
                internal_context = internal_context.add_value(
                    y2, interpret(internal_context, t2.default_value))
    ...
    if dependency is not None:
        if not interpret(internal_context, dependency):
            context = context.add_error(
                f"Dependency {dependency} not satisfied in {node_instance.name.name}"
            )
    # ##################################################################################################################

    result_connections: list[
        Union[Publisher, Subscriber, Service, Action,
              TFTransform]] = copy.deepcopy(x2_node_type.connections +
                                            x2_node_type.frames)
    ...
    for name, ttype in internal_context.typing.items():
        if isinstance(ttype,
                      PluginT) and name in internal_context.values.values():
        ...
        for name, value in internal_context.values.items():
            if isinstance(connection, TFTransform):
                substitute_connection_with_assignments(
                    internal_context, connection, name,
                    Literal(value=value, ttype=t_bottom))
            else:
                substitute_connection_with_assignments(
                    internal_context, connection, name,
                    Literal(value=value, ttype=connection.topic.ttype))

        for old_name, new_name in remaps:  # TODO: WIP
            substitute_connection_with_remaps(connection, old_name, new_name)

    return context.add_connections(name=node_instance.name.name,
...
def d_plugin_type(context: Context, plugin_type: PluginType) -> Context:
    ...
    if plugin_type.dependency is not None:
        pluginty = RefinedType(name=plugin_type.name,
                               ttype=pluginty,
                               refinement=plugin_type.dependency)
...
def d_plugin_instance(context: Context, plugin_instance: PluginInstance):
    ...
    if plugin_instance.name.name in context.typing:
        return context.add_error(
            f"Plugin {plugin_instance.name.name} already defined in context")
    ...
    x2_plugin_type = internal_context.get_typing(
        plugin_instance.plugin_ttype.name)
    assert isinstance(x2_plugin_type, PluginT) or isinstance(
        x2_plugin_type, RefinedType)
    ...
    if not is_subtype(internal_context, plugin_instance_st,
                      x2_plugin_type.fields):
        context = context.add_error(
    ...
    if dependency is not None:
        if not interpret(internal_context, dependency):
            context = context.add_error(
                f"Dependency {dependency} not satisfied in {plugin_instance.name.name}"
            )
    # ##################################################################################################################

    result_connections: list[Union[Publisher, Subscriber, Service,
                                   Action]] = copy.deepcopy(
                                       x2_plugin_type.connections)
    ...
    for connection in result_connections + result_frames:
        for name, value in internal_context.values.items():
            subs_ttype = t_bottom if isinstance(
                connection, TFTransform) else connection.topic.ttype
            substitute_connection_with_assignments(
                internal_context, connection, name,
                Literal(value=value, ttype=subs_ttype))
    ...
    return context.add_typing(
        ...
        PluginT(
            fields=StructType(
                {}
            ),  # There is no reason to keep the fields outside of plugin instance for now
            connections=result_connections,
...
def d_system(context: Context, system: System) -> Context:
    ...
    for node, all_connections in context.connections.items():
        system_connections.extend([(node, connection)
    ...
    for node, connection in system_connections:
        if isinstance(connection, Subscriber):
            if isinstance(connection.topic.ttype, RefinedType):
                ...
                pub_connections = interpret_connection(context,
                                                       connection.topic,
                                                       Publisher)

                if len(pub_connections) == 0:
                    context = context.add_error(
                        f"Publisher not found for subscriber {connection.topic}"
                    )

                for conn2 in pub_connections:
                    if not is_subtype(context, conn2.topic.ttype,
                                      connection.topic.ttype):
                        context = context.add_error(
                    ...
                    check_policies(context, connection.policies,
                                   conn2.policies)
        ...
        if isinstance(connection, Service):
            if isinstance(connection.topic.ttype, RefinedType):
                ...
                provider_connections = interpret_connection(
                    context, connection.topic, Service)
                provider_connections = [
                    x for x in provider_connections
                    if x.role == ServiceActionRole.PROVIDES
                ]

                if len(provider_connections) == 0:
                    context = context.add_error(
                        f"Provider not found for service {connection.topic}")
        ...
        if isinstance(connection, Action):
            if isinstance(connection.topic.ttype, RefinedType):
                ...
                provider_connections = interpret_connection(
                    context, connection.topic, Action)
                provider_connections = [
                    x for x in provider_connections
                    if x.role == ServiceActionRole.PROVIDES
                ]
        ...
        if isinstance(connection, TFTransform):
            if connection.transform == TransformType.LISTEN:
                ...
                for conn2 in system_connections[index + 1:]:
                    if isinstance(
                            conn2, TFTransform
                    ) and conn2.transform == TransformType.BROADCAST:
                        broadcaster_child = interpret(context,
                                                      conn2.child_frame)
                        broadcaster_parent = interpret(context,
                                                       conn2.parent_frame)
            ...
            if connection.transform == TransformType.BROADCAST:
                ...
                for node2, conn2 in system_connections[index + 1:]:
                    if isinstance(
                        ...
                        broadcast2_child = interpret(context,
                                                     conn2.child_frame)
                        broadcast2_parent = interpret(context,
                                                      conn2.parent_frame)
