...
def get_test_cases(test_type: str):
    ...
    cases = []
    for rospec_path in glob.glob(
            os.path.join(evaluation_dir, f"{test_type}-*.rospec")):
...
@pytest.mark.parametrize("rospec_path, expected_path, case_id",
                         get_test_cases("detectable"))
